...
- Comparison of pseudo‑random, stratified, and low‑discrepancy sampling
...
- Divides the unit square into strata (subregions) and samples uniformly within each
- Reduces variance compared to standard MC
- Provides smoother convergence and better error properties

### 3. Streaming Monte Carlo
...
1. Compute single MC, stratified MC, and Sobol estimates
2. Compute multi-run statistics
3. Plot streaming convergence
4. Plot error vs sample size

## Example Usage

```python
from estimators import estimate_pi_mc, estimate_pi_stratified_mc
from sequences import estimate_pi_sobol

pi_mc = estimate_pi_mc(1_000_000)
pi_strat = estimate_pi_stratified_mc(256*256)
